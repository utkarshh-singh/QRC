...
def regression_metrics(y_test, predictions):
    ...
    # Normalized RMSE (by std); add eps for safety
    eps = 1e-12
    nrmse_std = rmse / (np.std(y_test) + eps)

    # Optional: normalized by range
    nrmse_range = rmse / ((np.max(y_test) - np.min(y_test)) + eps)
...
def plot_actual_vs_pred(y_test, predictions, title_prefix=""):
    y_test = np.asarray(y_test, dtype=float)
    predictions = np.asarray(predictions, dtype=float)

    # --- 1) Actual vs Predicted over sample index ---
    plt.figure(figsize=(10, 4))
    plt.plot(y_test, label="Actual (y_test)")
    plt.plot(predictions, label="Predicted", linestyle="--")
    plt.xlabel("Test sample index")
    plt.ylabel("Target value")
    plt.title(f"{title_prefix} Actual vs Predicted (ordered)")
    plt.legend()
    plt.tight_layout()
    plt.show()

    # --- 2) Scatter plot with y=x reference line ---
    ...
    plt.title(f"{title_prefix} Predicted vs Actual (scatter)")
    plt.tight_layout()
    plt.show()
