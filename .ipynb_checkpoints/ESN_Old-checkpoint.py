...
class ESNetwork:
    ...
    def __init__(self, reservoir, dim=4, regularization=1e-6, alpha=0.8, show_progress=True, 
                 approach='feedback', model_type='ridge', comb=None, limit=None, cpk=False, save_states=True):
        """
...
#         :param y_test: Test targets for NARMA10 task.
#         """
#         X_reservoir = self.model.fit(X)  # Transform input through reservoir
#         X_train_reservoir = self.fit(X_train)
#         X_test_reservoir = self.fit(X_test)
...
# Example:
# from reservoirs import CPRC , GBSampling, GBPermanents
# from utils import ReservoirWrapper
# # Examples
# gb_permanents1 = GBPermanents(network_connections=[[3, 2], [2, 1], [3, 2], [2, 1]], classical=True, kernel=False)
# gb_permanents2 = GBPermanents(network_connections=[[3, 2], [2, 1], [3, 2], [2, 1]], classical=False, kernel=False)
# gb_sampling = GBSampling(RPhases=0, value=1, cutoff=40, avgPhotons=2)
# cprc = CPRC(dim=4, execution_mode='simulation')

# r1 = ReservoirWrapper(gb_permanents1)
# r2 = ReservoirWrapper(gb_permanents2)
# r3 = ReservoirWrapper(gb_sampling)
# r4 = ReservoirWrapper(cprc)

# res1 = r1.compute(x)
# res2 = r2.compute(x)
# res3 = r3.compute(x)
# res4 = r4.compute(x)
# print(f"Output of reservoir system 1: {res1}")
# print(f"Output of reservoir system 2: {res2}")
# print(f"Output of reservoir system 3: {res3}")
# print(f"Output of reservoir system 4: {res4}")

class EchoStateNetwork:
    def __init__(self, reservoir=None, dim=4, regularization=1e-6, alpha=0.8, show_progress=True, approach='feedback', model_type='ridge'):
        """
        Universal Echo State Network supporting different reservoir functions and regression models.
        
        Parameters:
            reservoir: An instance of ReservoirWrapper or None (for pure regression without reservoirs).
            dim (int): Fixed input size for the reservoir device.
            regularization (float): Regularization parameter for Ridge/Lasso.
            alpha (float): Feedback memory parameter (0 to 1). Higher values retain more memory.
            show_progress (bool): Whether to display a progress bar.
            approach (str): 'feedback' or 'time-multiplexing'.
            model_type (str): Regression model type ('ridge', 'lasso', 'linear', 'svr').
        """
        self.reservoir = ReservoirWrapper(reservoir)
        self.dim = dim
        self.alpha = alpha
        ...
        self.model_type = model_type.lower()
        self.model = None
        self.prev_output = np.zeros(dim)  # Initialize previous output memory

    def _initialize_model(self):
        """Initialize the chosen regression model."""
        if self.model_type == 'ridge':
            ...
            raise ValueError("Invalid model_type. Choose from 'ridge', 'lasso', 'linear', 'svr'.")

    def fit(self, X, Y):
        """
        Train the reservoir system or pure regression model if no reservoir is used.
        ...
        for x in iterator:
            ...
            else:  # Time-multiplexing
                modified_input = x
            
            transformed_state = self.reservoir.compute(modified_input) if self.reservoir else modified_input
            transformed_states.append(transformed_state)
            
            if self.approach == 'feedback':
                prev_output = transformed_state  # Update feedback memory
        
        transformed_states = np.array(transformed_states)
        if transformed_states.ndim == 1:
            transformed_states = transformed_states.reshape(-1, 1)
        
        self.model = self._initialize_model().fit(transformed_states, Y)

    def predict(self, X, initial_input=None, n=None, m=None, X_test=None):
        """
        Predict output using the trained model. Can perform standard prediction or sequential prediction for next n values.
        
        Parameters:
            X (array-like): Input time series data.
            initial_input (array-like, optional): Initial sequence for sequential predictions.
            n (int, optional): Number of future values to predict.
            m (int, optional): Reset interval (after how many predictions to reset to X_test).
            X_test (array-like, optional): Test data for periodic resetting.
        ...
        if n is None:  # Standard prediction
            transformed_states = []
            prev_output = np.zeros(self.dim)  # Initialize feedback memory
            ...
            for x in iterator:
                ...
                if self.approach == 'feedback':
                    prev_output = transformed_state
            
            transformed_states = np.array(transformed_states)
            if transformed_states.ndim == 1:
                transformed_states = transformed_states.reshape(-1, 1)
            
            return self.model.predict(transformed_states)
        
        # Sequential prediction (next n values)
        predictions = []
        current_input = np.array(initial_input).reshape(1, -1)
        test_index = 0  # Track X_test reset index
        ...
        for i in iterator:
            if self.approach == 'feedback':
                ...
                modified_input = current_input[0]
            
            transformed_state = np.array([self.reservoir.compute(modified_input)]) if self.reservoir else np.array([modified_input])
            if transformed_state.ndim == 1:
                transformed_state = transformed_state.reshape(-1, 1)
            
            next_pred = self.model.predict(transformed_state)
            predictions.append(next_pred.flatten()[0])
            
            prev_output = transformed_state.flatten()
